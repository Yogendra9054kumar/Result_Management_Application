...
class CourseClass:
    def __init__(self,root):
        ...
        # ==========Title============
        title=Label(self.root,text="Manage Course Details",font=('goudy old style',30,'bold'),bg='#033054',fg='white').place(x=10,y=15,width=1180,height=35)
        ...
        lbl_duration = Label(self.root,text="Duration Name", font=("goudy old style",15,"bold"),bg='white').place(x=10,y=100)
        ...
        txt_search_courseName = Entry(self.root,textvariable=self.var_search,font=("goudy old style",15,"bold"),bg='lightyellow').place(x=850,y=60,width=200)
    ...
    def delete(self):
        ...
        try:
            if self.var_course.get() =="":
                ...
                cursr.execute("select*from course where name=?",(self.var_course.get(),))
                row = cursr.fetchone()
                if row == None:
                    ...
                    if op==True:
                        cursr.execute("delete from course where name=?",(self.var_course.get(),))
                        con.commit()
        ...
        try:
            if self.var_course.get() =="":
                ...
                if row!= None:
                    ...
                    cursr.execute("insert into course (name,duration ,charges,description) values(?,?,?,?)",(
        ...
        try:
            if self.var_course.get() =="":
                ...
                if row == None:
                    ...
                    cursr.execute("update course set duration=? ,charges=?,description=? where name=?",(
    ...
    def show(self):
        ...
        cursr = con.cursor()
        try:
            cursr.execute("select*from course ")
            rows = cursr.fetchall()
    ...
    def search(self):
        ...
        cursr = con.cursor()
        try:
            cursr.execute(f"select*from course where name LIKE '%{self.var_search.get()}%'")
            rows = cursr.fetchall()
            self.CourseTable.delete(*self.CourseTable.get_children())
            for row in rows:
                self.CourseTable.insert('',END,values=row)
